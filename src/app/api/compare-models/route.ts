...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const confidences = [claudeResult.confidence, gpt4Result.confidence, geminiResult.confidence]
    const avgConfidence = confidences.reduce((a, b) => a + b, 0) / 3
    const stdDev = Math.sqrt(
      confidences.reduce((sum, c) => sum + Math.pow(c - avgConfidence, 2), 0) / 3
    )
    ...
    return NextResponse.json({
      ...
      comparison: {
        ...
        metrics: {
          ...
          fastest_model: [claudeResult, gpt4Result, geminiResult]
            .sort((a, b) => a.processing_time_ms - b.processing_time_ms)[0].model,
          highest_confidence: [claudeResult, gpt4Result, geminiResult]
            .sort((a, b) => b.confidence - a.confidence)[0].model,
        },
      },
    })
...
export async function GET() {
  return NextResponse.json({
    ...
    outputs: {
      models: 'Array of responses from each model',
